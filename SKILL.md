...
├── project-overview.md          # 项目总览（目标：3-5 分钟读完）
├── getting-started.md           # 安装、配置、启动（目标：15 分钟上手）
├── feature-summary.md           # 功能概括（目标：功能全景图）
├── <domain>.md                  # 领域/功能域文档（如 network.md, storage.md）
└── <feature>.md                 # 具体功能点文档（如 peer-management.md, routing.md）
```

### 文档拆分原则

**单篇篇幅控制**：
- 项目总览/入门指南/功能概括：≤ 200 行（约 8KB）
- 单个模块/功能文档：≤ 300 行（约 12KB）
- 超出则必须拆分
...
| 场景 | 拆分方式 | 示例 |
|------|----------|------|
| 模块功能过多 | 按功能点拆分为独立文档 | `peers.md` → `peer-management.md` + `routing.md` + `crypto.md` |
| 功能点过于复杂 | 拆分为概念文档 + 实现文档 | `routing.md` → `routing-concept.md` + `routing-impl.md` |
| 跨模块功能 | 单独成篇，引用相关模块 | `nat-traversal.md`（涉及 peers + tunnel + gateway）|

**命名约定**：
- 领域文档：名词形式，如 `network.md`, `storage.md`, `security.md`
- 功能文档：动词-名词形式，如 `peer-management.md`, `packet-routing.md`, `config-sync.md`
- 避免使用模块内部类型名作为文档名（如不要用 `peerconn.md`，用 `peer-connection.md`）
...
**目的**：深入理解特定功能或领域的实现细节和设计决策。

**文档粒度**：
- 每个文档聚焦**一个功能点**或**一个功能域**
- 单篇篇幅 ≤ 300 行，超出则拆分
...
- **篇幅控制**：如果一个功能需要超过 300 行描述，说明应该拆分为多个功能文档
...
- 功能/领域文档 >= 3
- 每篇功能文档包含完整四要素：实现细节、设计原理、代码片段、涉及模块
- 强制要求"涉及模块"下有逐模块子章节
